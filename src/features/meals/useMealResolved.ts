...
/** Meal + items + their foods + per-portion totals. Hook-free, so callers
 *  outside React can use it too - logging a reserved meal, for one. */
...
export function useMealResolved(mealId: string | undefined): ResolvedMeal | null | undefined {
  return useLiveQuery<ResolvedMeal | null>(() => resolveMeal(mealId), [mealId]);
}
